 ...
 * Opérations CRUD communes avec cache et synchronisation
 */
...
export abstract class BaseRepository<T extends BaseEntity> {
  ...
  async update(id: string, updates: Partial<Omit<T, 'id' | 'createdAt'>>): Promise<T | null> {
    ...
    try {
      ...
      // Marquer comme non-synchronisé en supprimant syncedAt
...
/**
 * Mixin pour ajouter des capacités de recherche
 */
export function WithSearch<T extends BaseEntity>(Base: typeof BaseRepository) {
  return class extends Base<T> {
    async search(
      query: string,

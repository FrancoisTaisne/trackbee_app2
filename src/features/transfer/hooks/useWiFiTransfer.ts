...
/**
 * WiFi Transfer Hook
 * Hook spécialisé pour les transferts WiFi
 */
...
type WiFiDeviceInfo = any
type WiFiTransferOptions = any
type Transfer = any
type UseWiFiTransferReturn = any

// PUSH FINAL: Constants temporaires avec any
const transferQueryKeys: any = {}
const WiFiTransferOptionsSchema: any = {}
const WIFI_DEFAULT_SETTINGS: any = {}
...
async function testWiFiConnection(ipAddress: string, port: number = 80): Promise<{
  isReachable: boolean
  latency?: number
  error?: string
}> {
  ...
  const result = await httpClient.post<{
    ...
    latency?: number
    error?: string
  }>('/api/transfers/wifi/test', { ipAddress, port })
...
async function getLocalNetworkInfo(): Promise<{
  ...
  try {
    // Essayer d'obtenir les infos réseau via l'API
    const networkInfo = await httpClient.get<{
      ssid?: string
      ipAddress?: string
      gateway?: string
      subnet?: string
    }>('/api/transfers/wifi/network-info')

    log.debug('Local network info', { networkInfo })
    return networkInfo
  } catch (error) {
...
export function useWiFiTransfer(): UseWiFiTransferReturn {
  ...
  const scanMutation = useMutation({
    mutationFn: async () => {
      ...
      try {
        // Combiner scan réseau local et devices connus
        const [networkDevices, knownDevices] = await Promise.all([
          scanLocalNetwork(),
          scanForWiFiDevices()
        ])

        // Merger et déduplicater par IP
  ...
  const connectMutation = useMutation({
    mutationFn: async ({ ipAddress, options }: { ipAddress: string, options?: Partial<WiFiTransferOptions> }) => {
  ...
  const disconnectMutation = useMutation({
    mutationFn: disconnectWiFiDevice,
    onSuccess: () => {
      refetchStatus()
  ...
  const testConnectionMutation = useMutation({
    mutationFn: ({ ipAddress, port }: { ipAddress: string, port?: number }) =>
      testWiFiConnection(ipAddress, port),
    onError: (error: Error) => {
      setLastError(error)
  ...
  const downloadMutation = useMutation({
    mutationFn: ({ fileName, options }: { fileName: string, options?: Partial<WiFiTransferOptions> }) =>
      downloadWiFiFile(fileName, options),
    onSuccess: (transfer) => {
  ...
  const uploadMutation = useMutation({
    mutationFn: ({ file, options }: { file: File, options?: Partial<WiFiTransferOptions> }) =>
      uploadWiFiFile(file, options),
    onSuccess: (transfer) => {
      queryClient.invalidateQueries({ queryKey: transferQueryKeys.lists() })
